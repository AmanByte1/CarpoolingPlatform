...
router.post('/', protect, async (req, res) => {
  try {
    ...
    // Date check here (not in schema) so seed scripts can write historical data freely
    ...
    if (depDate < todayStart || depDate > oneYearFromNow) {
      return res.status(400).json({ message: 'Departure date must be today or within 1 year from now' });
    }
